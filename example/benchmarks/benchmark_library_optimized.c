...
void library_benchmark_init(LibraryBenchmark *lb, int width, int height) {
  ...
  for (int i = 0; i < MAX_OBJECTS; i++) {
    ...
    float x =
        (col - 2) * 20.0f + ((float)rand() / (float)RAND_MAX - 0.5f) * 10.0f;
    float y = -7.0f + ((float)rand() / (float)RAND_MAX - 0.5f) * 5.0f;
    float z =
        -30.0f - row * 20.0f + ((float)rand() / (float)RAND_MAX - 0.5f) * 15.0f;

    lb->positions[i] = (Vec3f){x, y, z};

    // Random rotation and rotation speed
    lb->rotations[i] = (Vec3f){((float)rand() / (float)RAND_MAX) * 2.0f * M_PI,
                               ((float)rand() / (float)RAND_MAX) * 2.0f * M_PI,
                               ((float)rand() / (float)RAND_MAX) * 2.0f * M_PI};

    lb->rotation_speeds[i] = ((float)rand() / (float)RAND_MAX) * 2.0f + 0.5f;
  ...
  while (elapsed < duration) {
    ...
    if (lb->frame_count % 30 == 0) {
      printf("  Frame %d - FPS: %.1f (rendering %d objects)\n", lb->frame_count,
             lb->frame_count / elapsed, MAX_OBJECTS);
    }
  }
...
int main() {
  ...
  printf("conditions where culling optimizations can demonstrate their "
         "value.\n\n");
