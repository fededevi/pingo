...
void visual_benchmark_update_scene(VisualBenchmark *vb) {
  ...
  vb->avg_transform_time =
      (vb->avg_transform_time * vb->frame_count + transform_time) /
      (vb->frame_count + 1);
}
...
void visual_benchmark_render_frame(VisualBenchmark *vb) {
  ...
  vb->avg_render_time = (vb->avg_render_time * vb->frame_count + render_time) /
                        (vb->frame_count + 1);
  ...
  if (vb->frame_count % 60 == 0) {
    printf("Frame %d - FPS: %.1f - Transform: %.4fms - Render: %.4fms\n",
           vb->frame_count, vb->fps, vb->avg_transform_time * 1000.0,
