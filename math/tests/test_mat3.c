...
int test_mat3(void) {
  ...
  TEST_ASSERT(is_only_trans == 1,
              "mat3IsOnlyTranslation failed for translation matrix");

  int is_only_trans_identity = mat3IsOnlyTranslation(&identity);
  TEST_ASSERT(is_only_trans_identity == 1,
              "mat3IsOnlyTranslation failed for identity matrix");

  int is_only_trans_scale = mat3IsOnlyTranslation(&scale);
  TEST_ASSERT(is_only_trans_scale == 0,
              "mat3IsOnlyTranslation failed for scale matrix");
