  ...
  do {                                                                         \
    if (fabs((expected).x - (actual).x) > (tolerance) ||                       \
      ...
      printf("FAIL: Expected Vec2f(%f, %f), got Vec2f(%f, %f) at %s:%d\n",     \
             (expected).x, (expected).y, (actual).x, (actual).y, __FILE__,     \
             __LINE__);                                                        \
      return 0;                                                                \
    }                                                                          \
  } while (0)
...
#define TEST_ASSERT_VEC3F_EQ(expected, actual, tolerance)                      \
  do {                                                                         \
    if (fabs((expected).x - (actual).x) > (tolerance) ||                       \
      ...
      printf("FAIL: Expected Vec3f(%f, %f, %f), got Vec3f(%f, %f, %f) at "     \
             "%s:%d\n",                                                        \
             (expected).x, (expected).y, (expected).z, (actual).x, (actual).y, \
             (actual).z, __FILE__, __LINE__);                                  \
      return 0;                                                                \
    }                                                                          \
  } while (0)
    ...
    for (int i = 0; i < 9; i++) {                                              \
      if (fabs((expected).elements[i] - (actual).elements[i]) > (tolerance)) { \
        printf("FAIL: Mat3 elements[%d] expected %f, got %f at %s:%d\n", i,    \
               (expected).elements[i], (actual).elements[i], __FILE__,         \
               __LINE__);                                                      \
        return 0;                                                              \
      }                                                                        \
    }                                                                          \
...
// Runs a single suite by name. Returns 1 if it passed, 0 if it failed, and -1
// if no suite goes by that name.
int run_test_by_name(const char *name);

// Prints the available suite names, space separated, without a newline.
