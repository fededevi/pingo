...
int test_vec3(void) {
  ...
  F_TYPE length =
      sqrtf(normalized.x * normalized.x + normalized.y * normalized.y +
  ...
  // vec3Normalize has no unit vector to return here, and documents the
  // zero vector as its own result rather than producing NaNs.
  Vec3f zero_norm = vec3Normalize(zero);
  TEST_ASSERT_VEC3F_EQ(zero, zero_norm, 0.001f);
