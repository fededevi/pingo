...
int test_functions(void) {
  ...
  TEST_ASSERT(edge_result < 0,
              "edgeFunction failed for counter-clockwise triangle");
  ...
  TEST_ASSERT(orient_result > 0,
              "orient2d failed for counter-clockwise triangle");

  Vec2i d_int = {1, 1};
  int orient_result2 = orient2d(a_int, b_int, d_int);
  TEST_ASSERT(orient_result2 > 0,
              "orient2d failed for counter-clockwise triangle");
