...
int renderer_init(Renderer *r, Vec2i size, Backend *backend) {
  ...
  const RenderTarget *t = backend->get_target(r, backend);
  ...
  return render_target_init(&r->target, size, t->color.pixels, t->depth);
}
...
int renderer_render(Renderer *r) {
  ...
  const RenderTarget *t = be->get_target(r, be);
  IF_NULL_RETURN(t, RENDER_ERROR);
  r->target.color.pixels = t->color.pixels;
