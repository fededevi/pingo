...
int rasterizer_draw_transformed(Mat4 t, Renderer *r, Texture *src) {
  ...
  // Now we can iterate over the pixels of the axis-alignes-bounding-box (AABB)
  // which contain the source frame
  for (int y = minY; y <= maxY; y++) {
    for (int x = minX; x <= maxX; x++) {
      ...
      // frame Probably there is a faster way of doing this by checking line
      // intersection with the sprite transformed quad but I don't care for now
      // because we do not use rotations as of now.
      if (srcPosF.x < 0)
        continue;
      if (srcPosF.y < 0)
        continue;
      if (srcPosF.x >= src->size.x)
        continue;
      if (srcPosF.y >= src->size.y)
        continue;
      Pixel color = texture_read(src, srcPosI);
#endif
      ...
      if (srcPosF.y < 0)
        continue;
      if (srcPosF.x >= src->size.x - 1)
        continue;
      if (srcPosF.y >= src->size.y - 1)
        continue;
      ...
      Vec2i srcPosI1 =
          vec2iClamp((Vec2i){srcPosF1.x, srcPosF1.y}, (Vec2i){0, 0}, src->size);
      Vec2i srcPosI2 =
          vec2iClamp((Vec2i){srcPosF1.x, srcPosF2.y}, (Vec2i){0, 0}, src->size);
      Vec2i srcPosI3 =
          vec2iClamp((Vec2i){srcPosF2.x, srcPosF1.y}, (Vec2i){0, 0}, src->size);
      Vec2i srcPosI4 =
          vec2iClamp((Vec2i){srcPosF2.x, srcPosF2.y}, (Vec2i){0, 0}, src->size);

      float p1 =
          src->frameBuffer[(int)(srcPosI1.x + srcPosI1.y * src->size.x)].g;
      float p2 =
          src->frameBuffer[(int)(srcPosI2.x + srcPosI2.y * src->size.x)].g;
      float p3 =
          src->frameBuffer[(int)(srcPosI3.x + srcPosI3.y * src->size.x)].g;
      float p4 =
          src->frameBuffer[(int)(srcPosI4.x + srcPosI4.y * src->size.x)].g;
      ...
      if (srcPosF1.x < 0 && srcPosF2.x < 0 && srcPosF3.x < 0 && srcPosF4.x < 0)
        continue;
      if (srcPosF1.y < 0 && srcPosF2.y < 0 && srcPosF3.y < 0 && srcPosF4.y < 0)
        continue;
      if (srcPosF1.x > src->size.x && srcPosF2.x > src->size.x &&
          srcPosF3.x > src->size.x && srcPosF4.x > src->size.x)
        continue;
      if (srcPosF1.y > src->size.y && srcPosF2.y > src->size.y &&
          srcPosF3.y > src->size.y && srcPosF4.y > src->size.y)
        continue;
      ...
      Vec2i srcPosI2 =
          vec2iClamp((Vec2i){srcPosF1.x, srcPosF2.y}, (Vec2i){0, 0}, src->size);
      Vec2i srcPosI3 =
          vec2iClamp((Vec2i){srcPosF1.x, srcPosF3.y}, (Vec2i){0, 0}, src->size);
      Vec2i srcPosI4 =
          vec2iClamp((Vec2i){srcPosF1.x, srcPosF4.y}, (Vec2i){0, 0}, src->size);
      Vec2i srcPosI5 =
          vec2iClamp((Vec2i){srcPosF2.x, srcPosF1.y}, (Vec2i){0, 0}, src->size);
      Vec2i srcPosI6 =
          vec2iClamp((Vec2i){srcPosF2.x, srcPosF2.y}, (Vec2i){0, 0}, src->size);
      Vec2i srcPosI7 =
          vec2iClamp((Vec2i){srcPosF2.x, srcPosF3.y}, (Vec2i){0, 0}, src->size);
      Vec2i srcPosI8 =
          vec2iClamp((Vec2i){srcPosF2.x, srcPosF4.y}, (Vec2i){0, 0}, src->size);
      Vec2i srcPosI9 =
          vec2iClamp((Vec2i){srcPosF3.x, srcPosF1.y}, (Vec2i){0, 0}, src->size);
      Vec2i srcPosI0 =
          vec2iClamp((Vec2i){srcPosF3.x, srcPosF2.y}, (Vec2i){0, 0}, src->size);
      Vec2i srcPosI11 =
          vec2iClamp((Vec2i){srcPosF3.x, srcPosF3.y}, (Vec2i){0, 0}, src->size);
      Vec2i srcPosI12 =
          vec2iClamp((Vec2i){srcPosF3.x, srcPosF4.y}, (Vec2i){0, 0}, src->size);
      Vec2i srcPosI13 =
          vec2iClamp((Vec2i){srcPosF4.x, srcPosF1.y}, (Vec2i){0, 0}, src->size);
      Vec2i srcPosI14 =
          vec2iClamp((Vec2i){srcPosF4.x, srcPosF2.y}, (Vec2i){0, 0}, src->size);
      Vec2i srcPosI15 =
          vec2iClamp((Vec2i){srcPosF4.x, srcPosF3.y}, (Vec2i){0, 0}, src->size);
      Vec2i srcPosI16 =
          vec2iClamp((Vec2i){srcPosF4.x, srcPosF4.y}, (Vec2i){0, 0}, src->size);
      ...
      float p4 =
          src->frameBuffer[(int)(srcPosI4.x + srcPosI4.y * src->size.x)].g;
      float p5 =
          src->frameBuffer[(int)(srcPosI5.x + srcPosI5.y * src->size.x)].g;
      float p6 =
          src->frameBuffer[(int)(srcPosI6.x + srcPosI6.y * src->size.x)].g;
      float p7 =
          src->frameBuffer[(int)(srcPosI7.x + srcPosI7.y * src->size.x)].g;
      float p8 =
          src->frameBuffer[(int)(srcPosI8.x + srcPosI8.y * src->size.x)].g;
      float p9 =
          src->frameBuffer[(int)(srcPosI9.x + srcPosI9.y * src->size.x)].g;
      float p0 =
          src->frameBuffer[(int)(srcPosI0.x + srcPosI0.y * src->size.x)].g;
      float p11 =
          src->frameBuffer[(int)(srcPosI11.x + srcPosI11.y * src->size.x)].g;
      float p12 =
          src->frameBuffer[(int)(srcPosI12.x + srcPosI12.y * src->size.x)].g;
      float p13 =
          src->frameBuffer[(int)(srcPosI13.x + srcPosI13.y * src->size.x)].g;
      float p14 =
          src->frameBuffer[(int)(srcPosI14.x + srcPosI14.y * src->size.x)].g;
      float p15 =
          src->frameBuffer[(int)(srcPosI15.x + srcPosI15.y * src->size.x)].g;
      float p16 =
          src->frameBuffer[(int)(srcPosI16.x + srcPosI16.y * src->size.x)].g;

      Pixel color = pixelFromUInt8((p1 + p2 + p3 + p4 + p5 + p6 + p7 + p8 + p9 +
                                    p0 + p11 + p12 + p13 + p14 + p15 + p16) /
                                   16);
#endif
      texture_draw(&des, desPos, color);
    }
